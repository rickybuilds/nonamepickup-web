...
async function live() {
  ...
  set(
    `${pageHead("01", "Live", "Queue and game server state from the latest check.")}<div class="profile-ribbon"><span>QUEUE / ${fmt(q.count)} OF ${fmt(q.max)}</span><span>CHECKED ${checked}</span></div><div class="data-strip"><div class="strip-cell"><span class="micro">PLAYERS QUEUED</span><strong>${fmt(q.count)}</strong></div><div class="strip-cell"><span class="micro">SLOTS REMAINING</span><strong>${fmt(Math.max(0, (q.max || 8) - (q.count || 0)))}</strong></div><div class="strip-cell"><span class="micro">ACTIVE MATCHES</span><strong>${fmt(matches.length)}</strong></div><div class="strip-cell"><span class="micro">QUEUE CAPACITY</span><strong>${fmt(q.max || 8)}</strong></div></div>${section("02", "On the server")}${matches.length ? matches.map((m) => `<div class="match-row"><span class="match-date">${esc(m.serverKey || "SERVER")}</span><span class="match-map">${esc(m.map_name || m.map || "Map unknown")}</span><span class="score"><span class="blue">${esc(m.score_blue ?? m.blue_score ?? "—")}</span> : <span class="red">${esc(m.score_red ?? m.red_score ?? "—")}</span></span><span class="tag">${esc(m.timeleft || "IN PROGRESS")}</span><a class="row-arrow" href="${old("live.html")}" aria-label="Open live spectator">↗</a></div>`).join("") : message("No match is active on a connected server.")} ${section("03", "In queue")}${q.players?.length ? q.players.map((p, i) => `<div class="stat-line"><span><span class="number muted">${String(i + 1).padStart(2, "0")} / </span>${esc(p.name || p.id)}</span><span class="number">QUEUED</span></div>`).join("") : message("No players are currently queued.")}<div class="action-row"><button type="button" id="refresh-live">Refresh snapshot ↻</button><a href="${old("live.html")}">Live spectator ↗</a><a href="${old("pickup-live.html")}">Browser live viewer ↗</a></div>`,
  );
...
function matchTable(rows) {
  return rows.length
    ? rows
        .map(
          (m) =>
            `<a class="table-row matches-table" href="${link("match", "id", m.id)}"><span class="number muted">${esc(m.id)}</span><span class="table-name">${esc(m.map_name || "Unknown")}<small class="table-mobile-meta">${date(m.created_at)} / ${esc(matchStatus(m))}</small></span><span class="number">${date(m.created_at)}</span>${score(m)}<span class="tag">${esc(matchStatus(m))}</span><span>↗</span></a>`,
        )
        .join("")
...
function mapTable(rows) {
  return rows.length
    ? rows
        .map(
          (m) =>
            `<a class="table-row maps-table" href="${link("map", "id", m.map)}"><span class="number muted">${String(m.order).padStart(2, "0")}</span><span class="table-name">${esc(m.map)}</span><span class="number" data-label="MATCHES">${fmt(m.games)}</span><span class="number">${fmt(m.avgScorePerTeam)}</span><span>↗</span></a>`,
        )
        .join("")
