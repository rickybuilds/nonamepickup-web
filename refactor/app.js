...
async function players() {
  ...
  set(
    `${pageHead("03", "Players", "Current ELO with match record and recent form.")}<div class="lede"><span>${fmt(rows.length)} QUALIFIED RATINGS / PRIVATE RATINGS OMITTED</span><span>RECORD WINDOW / ${state.period ? `${state.period} DAYS` : "ALL TIME"}</span></div><div class="control-bar"><input id="player-filter" type="search" placeholder="Filter qualified players" aria-label="Filter qualified players" value="${esc(state.playerQuery)}"><select id="period-filter" aria-label="Match record window"><option value="7" ${state.period === 7 ? "selected" : ""}>7-day record</option><option value="30" ${state.period === 30 ? "selected" : ""}>30-day record</option><option value="90" ${state.period === 90 ? "selected" : ""}>90-day record</option><option value="0" ${state.period === 0 ? "selected" : ""}>All-time record</option></select><button type="button" id="open-search">SEARCH ALL PLAYERS ↗</button></div><div class="rank-note">Standings require 10+ completed games all time. The record window changes games and form; ELO is current.</div><div class="table-head leaderboard-table players-table"><span>ORDER</span><span>PLAYER</span><span>ELO</span><span>GAMES</span><span>RECORD</span><span>RECENT</span></div><div id="player-table">${playerTable(filtered)}</div>`,
  );
