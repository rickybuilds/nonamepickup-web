...
This is a static, self-contained frontend at `refactor/index.html`. It uses its own `screen.css` and `app.js`; it does not load the production styles or scripts. Views use query strings (`/refactor/?view=matches`, `/refactor/?view=player&id=…`) so direct links and refreshes work with ordinary static file serving. Styles, scripts, and the favicon use paths relative to `/refactor/`. Data requests deliberately use the existing same-origin `/api/` endpoints. Links to specialized legacy tools use `../` paths. The shared player profile API now aggregates combat, class, and MVP stats across every Steam ID linked to the player's Discord ID; detailed kill events also include linked Steam IDs when an event has no Discord ID. No database schema changed.
...
- Navigation is a visible, ruled four/three section index with touch-sized rows below 1100px. Search remains in the brand row.
