...
function chart(rows, esc) {
  ...
  return `<div class="dossier-chart"><svg viewBox="0 0 900 225" role="img" aria-label="ELO over the last ${points.length} rated matches, from ${Math.round(values[0])} to ${Math.round(values.at(-1))}"><line x1="24" y1="38" x2="876" y2="38"/><line x1="24" y1="113" x2="876" y2="113"/><line x1="24" y1="188" x2="876" y2="188"/><polygon points="24,188 ${esc(coords)} 876,188"/><polyline points="${esc(coords)}"/><circle cx="876" cy="${188 - (values.at(-1) - low) * 150 / range}" r="5"/></svg><div><span>${Math.round(low)} LOW</span><strong>${Math.round(values.at(-1))} CURRENT</strong><span>${Math.round(high)} HIGH</span></div></div>`;
}
...
export async function renderPlayerDossier(helpers) {
  ...
  const mapTile = (m) => `<a href="${link("map", "id", m.map)}" class="dossier-map-tile" style="--map-color:${num(m.win_pct) >= 50 ? "var(--blue)" : "var(--red)"}"><span class="dossier-map-image"><img data-player-map="${esc(m.map)}" src="${imagePath(m.map)}" alt="" loading="lazy"><b>${esc(m.map)}</b></span><span class="dossier-map-stats"><strong>${fmt(m.win_pct)}%</strong><span>WIN RATE</span><small>${fmt(m.gp)} G / ${fmt(m.w)}-${fmt(m.l)}-${fmt(m.t)}</small></span><i class="dossier-map-meter"><i style="width:${Math.max(0, Math.min(100, num(m.win_pct)))}%"></i></i></a>`;
  ...
  const renderMatches = () => {
    ...
    document.querySelector("#dossier-matches").innerHTML = rows.length ? rows.map((m) => {
      ...
      return `<article class="dossier-match result-${outcome}"><a href="${link("match", "id", m.id)}" class="dossier-match-image"><img data-player-map="${esc(m.map_name)}" src="${imagePath(m.map_name)}" alt="" loading="lazy"><span>${esc(m.map_name || "Unknown map")}</span></a><div class="dossier-match-main"><div class="dossier-match-scoreline"><span class="dossier-outcome result-${outcome}">${{ W: "WIN", L: "LOSS", T: "TIE" }[outcome] || "RESULT"}</span><strong>${score}</strong><span class="dossier-delta ${num(m.delta) >= 0 ? "positive" : "negative"}">${r.hidden ? "PRIVATE ELO" : `${signed(m.delta)} ELO`}</span></div><p>${date(m.created_at)} / ${esc(m.id)}</p><div class="dossier-teams">${teamLine(mine, "WITH")}${teamLine(theirs, "AGAINST")}</div><nav aria-label="Reports for ${esc(m.id)}"><a href="${link("match", "id", m.id)}">2080 MATCH ↗</a>${validUrl(m.hampalyzer_url) ? `<a href="${esc(validUrl(m.hampalyzer_url))}" target="_blank" rel="noopener noreferrer">HAMPALYZER ↗</a>` : ""}${validUrl(m.tfcstats_url) ? `<a href="${esc(validUrl(m.tfcstats_url))}" target="_blank" rel="noopener noreferrer">TFCSTATS ↗</a>` : ""}</nav></div></article>`;
