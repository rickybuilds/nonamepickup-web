...
Live browser network verification on 2026-09-23: `/api/supporters` returned 14 string IDs, including `468578577537826831` (rufi0), `562854514860752897` (SLICK), and `151144734579097601` (EDEdDNEdDYFaN). Production rendered diamonds for the first two on the 2fort leaderboard and no diamond for `{GTO}Nomad` (`356923411567738881`). Imported comparison data for `conc_sev` included Squishy's Batcave `http://squishysbatcave.com/map/5/` and Church of Conc `https://churchofconc.servehalflife.com/CofC.php?tab=stats&subtab=speedruns`. The production server map catalogue returned 503 due to the missing `attempt_stats` join in its query; the unused select expression was removed in this pass.

After implementation, a local browser preview using the live production API rendered the `conc_sev` map and both exact source URLs, the `xpress2k4` internal record/replay ledger, runner profiles, search results, and match rosters. The live API identifies Moreno (`1291623909950754817`) as a supporter with private ELO; the preview showed the diamond beside Moreno with `PRIVATE` rating on the player file and match roster. The long runner name `EDEdDNEdDYFaN -z-` kept the diamond beside the final line on a 390px viewport. The server map inventory itself cannot be verified until the fixed API is deployed; the public endpoint still returned 503 after the branch push. The production chart remains linked as a specialized full-history instrument; 2080 renders the record-break ledger with attempts and improvement metadata. No database data or supporter state was changed.
