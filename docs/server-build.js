  ...
  { title: "Deploy New Server", body: [p("Deploy Ubuntu 26.04 LTS and login as root."), cmd("ssh root@<IP_ADDRESS>"), cmd("timedatectl set-timezone America/New_York\ntimedatectl")] },
{
  ...
  { title: "Copy Known-Good Server", body: [warn("Always copy /root/steamcmd from the newest known-good server, not an old/busted server."), cmd("rsync -avz --progress \\\nroot@66.55.67.73:/root/steamcmd/ \\\n/root/new/steamcmd/\n\nscp root@66.55.67.73:/root/new/hlds_run.sh /root/\nscp root@66.55.67.73:/root/new/hltv_run.sh /root/\nscp root@66.55.67.73:/root/new/tfc-health.js /root/")] },
  ...
  { title: "Create HLTV Directory", body: [cmd("mkdir -p /root/steamcmd/tfc/tfc/HLTV<SERVER_NAME>"), p("This must match HL_REMOTE_HLTV_DIR_<SERVER_NAME> in the bot .env.")] },
  { title: "Update Server.cfg", body: [p("update your server.cfg the Hostname and the HLTV Directory"), cmd("nano /root/steamcmd/tfc/tfc/server.cfg")]},
  { title: "Add in crontab", body: [cmd("crontab -e"), p("Make edits to HLTV<SERVER_NAME>"), cmd("1 9 * * * find /root/steamcmd/tfc/tfc/logs -type f -mtime +2 -delete >/dev/null>\n2 9 * * * find /root/steamcmd/tfc/tfc/HLTV<SERVER_NAME> -type f -mtime +2 -delete >/dev/null 2>&1\n3 9 * * * apt update && DEBIAN_FRONTEND=noninteractive apt -y upgrade && reboot\n")]},
